...
pub fn decode_shared_key(encoded: String) -> SharedSecret<NistP256> {
    SharedSecret::from(
        GenericArray::from_slice(base64::decode(encoded).unwrap().as_slice()).clone(),
    )
}
...
fn test_key_exchange_encode_decode() {
    ...
    let decoded1_to_2 = aes256_decrypt_string(&user2_to_1_shared_secret, &ciphertext1_to_2).unwrap();
    ...
    let decoded2_to_1 = aes256_decrypt_string(&user1_to_2_shared_secret, &ciphertext2_to_1).unwrap();
